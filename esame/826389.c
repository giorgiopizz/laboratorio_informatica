...
Per compilare è necessario includere la libreria matematica con -lm
*/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
lista * leggivalori(lista * lista){
        ...
        int i;
        for(i=0;i<K;++i){
                //nel caso l'input fosse da file è meglio eliminare
                //la riga seguente
...
int main(int argc, char * argv[]){
        ...
        printf("L'input deve essere del tipo x y m ");
        printf("ed invio per ogni punto\n");
        ...
        //la funzione libera percorre la lista liberando
        //la memoria allocata dinamicamente
        //per ogni singolo punto
        list=libera(list);
        //anche se la lista è stata liberata
        //è necessario liberare la memoria
        //allocata per i due pointer t e c
        free(list);
        return 0;
}
/*
è stata usata una lista concatenata invece di un array di punti perché
l'allocazione dinamica della memoria è un metodo più efficiente.
In questo programma non cambia molto per la memoria,
tuttavia per una futura implementazione o modifica di questo programma
si è preferito usare la lista(ad esempio se si dovesse eliminare un
elemento dalla lista o il numero di punti inserito fosse sconosciuto).
...
non è costretta a ripercorrerla ogni volta che viene aggiunto un
elemento
*/
